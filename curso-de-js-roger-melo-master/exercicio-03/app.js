...
//  const dogNames =   maleDogNames + femaleDogNames;
...
dogNames = maleDogNames.concat(femaleDogNames);
// console.log(dogNames)
...
armazenamento = [];
dogNames.forEach((nome) => {
    primeiraLetra = nome[0]
    armazenamento.push(primeiraLetra)
})
initials = armazenamento.join('');
...
*/
const emPalavra = dogNames.toString()
const dessert = `${emPalavra.substring(3, 5)}`+`${emPalavra.substring(8,10)}`+ `${emPalavra.substring(emPalavra.length-2, emPalavra.length)}`
const alterado = dessert.replace("n", "d");
// console.log(emPalavra)
// console.log(`A ${alterado} é um doce à base de coco, tradicional na América Latina e em Angola.`)
...
*/
let oddNumbers = [3, 5, 6];
let segundoItem = oddNumbers[1] + 4

console.log(segundoItem**3)
